...
{
  public static int TO_BE_PUSHED = 1;
  public static int TO_BE_PULLED = 2;
  public static int TO_BE_SYNC_RESPONSE = 3;
  ...
  */
  public static int msgCategory(MsgInfoSet mis)
  {
    if ( mis == null ) return TO_BE_PUSHED;

    String mep = Configuration.getMep(mis);
//    Binding binding = getBinding(mis);
//    String mep = binding.getMep();
    int ln = getLegNumber(mis);
      //getLegNumber(mep, binding.getFirstLeg().getAddress());
    if ( mep.equalsIgnoreCase(Constants.ONE_WAY_PUSH) ) return TO_BE_PUSHED;
    if ( mep.equalsIgnoreCase(Constants.ONE_WAY_PULL) ) return TO_BE_PULLED;
    if ( mep.equalsIgnoreCase(Constants.TWO_WAY_SYNC) )
    {
      if ( ln == 1 ) return TO_BE_PUSHED;
      else return TO_BE_SYNC_RESPONSE;
    }
    ...
    {
      if ( ln == 1 ) return TO_BE_PUSHED;
      else return TO_BE_PULLED;
    }
    if ( mep.equalsIgnoreCase(Constants.TWO_WAY_PULL_AND_PUSH) )
    {
      if ( ln == 2 ) return TO_BE_PULLED;
      else return TO_BE_PUSHED;
    }
  ...
  public static int getLegNumber(MsgInfoSet metadata)
  {
    if ( metadata == null ) return 1;
    if ( metadata.getLegNumber() != -1 ) return metadata.getLegNumber();
    String mep = Configuration.getMep( metadata.getPmode() );
    String address = Configuration.getAddress(metadata.getPmode(), 1);  
    int ln = getLegNumber(mep, address);
...
//    int ln = getLegNumber(mep, binding.getFirstLeg().getAddress());
  ...
  */
  private static int getLegNumber(String mep, String firstLegAddress)
  {
    if ( mep.equalsIgnoreCase(Constants.ONE_WAY_PUSH) ) return 1;
    if ( mep.equalsIgnoreCase(Constants.ONE_WAY_PULL) ) return 2;
    if ( mep.equalsIgnoreCase(Constants.TWO_WAY_SYNC) )
    {
      if ( EbUtil.isLocal(firstLegAddress, Constants.configContext) ) return 2;
      else return 1;
    }
    if ( mep.equalsIgnoreCase(Constants.TWO_WAY_PUSH_AND_PUSH) )
    {
    ...
    if ( mep.equalsIgnoreCase(Constants.TWO_WAY_PUSH_AND_PULL) )
    {
      if ( EbUtil.isLocal(firstLegAddress, Constants.configContext) ) return 3;
      else return 1;
    }
    if ( mep.equalsIgnoreCase(Constants.TWO_WAY_PULL_AND_PUSH) )
    {
