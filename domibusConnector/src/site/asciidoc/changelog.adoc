...
* Fix problem with messages incl. evidences report within UI when case sensitive DB (eg. MySQL) is used
* Fix problem when Initiator Role and Responder Role are different and the role is not defined by the
sending backend application. This fix enables the connector to retrieve this information from the uploaded p-modes.

After this fix has been applied, the p-Modes must be uploaded again!
