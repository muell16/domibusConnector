...
alter table DC_TRANSPORT_STEP
    add FINAL_STATE_REACHED TIMESTAMP;
...
-- AENDERUNG!: Die temp. Spalte darf nicht den Datentyp BIGINT haben weil die vorhandenen Werte vom Typ varchar sind
-- alter table DC_TRANSPORT_STEP change CONNECTOR_MESSAGE_ID bkp_cmid BIGINT NOT NULL;
alter table DC_TRANSPORT_STEP change CONNECTOR_MESSAGE_ID bkp_cmid VARCHAR(255) NOT NULL;
...
-- AENDERUNG!: Muss nicht konvertiert werden weil es den gleichen Datentyp hat
-- update      DC_TRANSPORT_STEP set           CONNECTOR_MESSAGE_ID=CONVERT(bkp_cmid, char);
update      DC_TRANSPORT_STEP set           CONNECTOR_MESSAGE_ID=bkp_cmid;
...
-- AENDERUNG!: DOMIBUS_CONNECTOR_MESSAGES existiert nicht
...
drop foreign key FK_MSGINFO_SERVICE;
