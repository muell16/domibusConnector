...
== Push/Pull connection prior to domibusConnector-4.4
...
== Push/Pull connection since domibusConnector-4.4

In version 4.4 of domibusConnector, a new way of pulling messages from the connector to the backend is implemented.
This new pulling mechanism allows the backend to decide the number of messages pulled from the connector. It also requires the backend 
to acknowledge the pulled messages in a second, asynchronous step.
...
This step is now split into three seperate calls:
...
* getMessageById		- each message must be downloaded seperately. The ids are received at the previous step. This allows the backend to control the number of messages pulling.
This causes the downloaded message to get into state PENDING_DOWNLOADED at the domibusConnector. 
* acknowledgeMessage 	- This method needs to be called for each message successfully downloaded after processing at the backend. Accoring to the result sent with this method call, the message
