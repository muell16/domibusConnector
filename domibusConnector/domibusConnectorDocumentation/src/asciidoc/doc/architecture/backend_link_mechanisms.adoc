...
//this chapter is a little bit confusing to me, but i also don't know how to point out the differences better between the 2 pull approaches.
...
In version 4.4 of domibusConnector, a new way of pulling messages from the connector to the backend is implemented (The old interface is still available).
...
Pulling a message is now split into three seperate calls:
