...
#PS C:\Entwicklung\repos\domibusConnector\domibusConnector\domibusConnectorLauncher\domibusConnectorStandaloneLauncher> java -D"loader.debug=true" -D"loader.path=./target/lib/" -D"c
#onnector.config.file=C:\Entwicklung\ecodexENV\EXECUser10\software\domibusConnector-tomcat\conf\connector\connector_oracle.properties" -cp ".\target\domibusConnectorStandaloneLaunch
#er-4.1.0-RC2-SNAPSHOT-standalone.jar" org.springframework.boot.loader.PropertiesLauncher
...
#"%JAVA_HOME%\bin\java" -D"spring.config.location=%CONFIG_FOLDER%" -D"loader.path=%LIB_FOLDER%" -cp "%CLASSPATH%" org.springframework.boot.loader.PropertiesLauncher
